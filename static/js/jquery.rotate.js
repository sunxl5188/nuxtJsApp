...
(function(k) {
		...
		d && (f = d.replace(/[tT]ransform/, "TransformOrigin"), f[0] == "T" && (f[0] = "t"));
		eval('IE = "v"=="\v"');
		jQuery.fn.extend({
			rotate(a) {
				if (this.length !== 0 && typeof a !== "undefined") {
					...
					for (var b = [], c = 0, d = this.length; c < d; c++) {
						...
						if (e.Wilq32 && e.Wilq32.PhotoEffect) e.Wilq32.PhotoEffect._handleRotation(a);
							...
							const f = k.extend(!0, {}, a);
								var e = (new Wilq32.PhotoEffect(e, f))._rootObj;
							b.push(k(e))
						}
					}
					return b
			...
			stopRotate() {
				for (let a = 0, b = this.length; a < b; a++) {
					const c = this.get(a);
		...
		Wilq32.PhotoEffect.prototype = {
			_setupParameters(a) {
				...
				typeof a.angle === "number" && (this._angle = a.angle);
			...
			_Loader: function() {
				return IE ?
					function() {
						let a = this._img.width;
							const b = this._img.height;
						this._imgWidth = a;
						this._imgHeight = b;
						...
						let a = Math.sqrt(this._imgHeight * this._imgHeight + this._imgWidth * this._imgWidth);
						this._width = 3 * a;
						this._height = 3 * a;
			...
			_animate() {
				let a = +new Date;
					const b = a - this._animateStartTime > this._parameters.duration;
